...
【图片与内容匹配原则——宁缺毋滥】
- 每个图片标记 [IMAGE_N] 必须紧放在它所描述的段落之后，不可错位到其他段落
- 图片提示词必须严格对应所在段落的具体内容：段落讲什么实验/场景，提示词就描述什么实验/场景
- 严禁使用与段落内容无关的笼统图片描述（如只写"科学实验"而不写具体是什么实验）
- 提示词中必须包含该段落中提到的关键物品、动作、场景元素
- 如果某个段落没有适合配图的具象场景，不要强行放图片标记
