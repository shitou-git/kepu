...
IMPORTANT: Output exactly {section_count} sections. Use the exact format above. ALL output must be in Chinese (中文), except proper nouns."""
...
- Write entirely in Chinese (中文)
- DO NOT include the section title in your output, only the body content
- At the very end, add [IMAGE] on its own line as the picture placeholder
""" + context
    ...
    def generate_image_prompts(self, sections: List[Dict], theme_name: str) -> List[str]:
        """根据每个 section 的标题和内容生成对应的图片提示词"""
        system_prompt = """You are a professional children's science illustrator, skilled at creating precise image prompts.

Requirements:
1. Each prompt must strictly correspond to the specific section content provided
...
Output format: One prompt per line, corresponding to each section in the given order."""

        sections_text = ""
        for i, section in enumerate(sections, 1):
            sections_text += f"Section {i}: {section['title']}\n{section['content'][:600]}\n\n"

        user_prompt = f"""Generate precise English image prompts for {len(sections)} sections of a "{theme_name}" article.

{sections_text}
Requirements:
1. One prompt per line, in the same order as the sections above
2. Each prompt must depict the main scene or concept of that specific section
3. Include characters, actions, environment details
4. Style: watercolor hand-drawn, cartoon characters, Pixar animation style, bright colors, suitable for 6-12 years old
5. Absolutely no realistic human photos or portraits
6. English description, detailed and specific"""
...
5. Output must be in Chinese (中文)"""
...
4. Output must be in Chinese (中文)"""
...
6. Output must be in Chinese (中文)"""
...
Output format: One title per line, {section_count} total, in order."""
    ...
    def generate_article(self, theme: str, theme_name: str, date_str: str) -> dict:
        ...
        for i, section in enumerate(outline["sections"], 1):
            ...
            # 清理 AI 可能误输出的格式说明文字
            if "[Content in Markdown" in text:
                text = text.replace("[Content in Markdown, with [IMAGE] marker at end]", "").strip()
        ...
        sections_for_prompts = [
            {"title": s["title"], "content": t}
            for s, t in zip(outline["sections"], section_texts)
        ]
        image_prompts = self.generate_image_prompts(sections_for_prompts, theme_name)
